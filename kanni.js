...
(function () {
  ...
  if ( !window.Kanni ) {
    ...
    window.Kanni = {
      ...
      'languages'       : {},     // All loaded languages
      //'enabledLanguages': {},     // Enabled languages, if not set, all loaded languages will be enabled by default
      'switchkey'       : 120,    // F9
    ...
    if (this.attachkeyevents) {
      ...
      if (this.attachclasses == '*') {
        ...
        for(var k=0;k<inputs.length;k++) {
          ...
          Kanni.enableNode(input);
        }

        var inputs = document.getElementsByTagName('textarea');
        for(var k=0;k<inputs.length;k++) {
          var textarea = inputs[k]
          Kanni.enableNode(textarea);
        }

      // attach to the specified classes.
      } else {
  ...
  Kanni.charExistsInMap = function(char, lang) {
    ...
    var config = this.langConfig(lang);
    if (!config) {
      return false;
    }
  ...
  Kanni.attachLanguageSwitcher = function (div) {
    ...
    // css only if not default
    if ( !div ) {
      css = css + '#kanni-lang-switch-block {padding:5px;text-align:left;z-index:99;line-height:1em;position:fixed;width:150px;bottom:0px;left:10px;background:#EDF5FA;border:solid 1px #336699;display:none;}';
    ...
    for (var lang in enabled_lans) {
      var langconfig = this.languages[lang];
      for (var method in enabled_lans[lang].methods) {
        var methodconfig = langconfig.methods[method];
